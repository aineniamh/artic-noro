...
parser.add_argument("--paf_file", action="store", type=str, dest="paf")
...
detailed_dict = {}            
with open(str(args.paf),"r") as f:
    for l in f:
        tokens=l.rstrip('\n').split()
        detailed_dict[tokens[0]]=tokens[5]
...
minimap_genotype_counter = collections.defaultdict(dict)
...
for record in SeqIO.parse(args.reads,"fastq"):
    ...
    if not amp=="none":
        try: 
            ...
            minimap_genotype_counter[amp][detailed_dict[record.id]]+=1
        except:
            genotype_counter[amp][top_amp[record.id]]=1
            minimap_genotype_counter[amp][detailed_dict[record.id]]=1
...
for amp in sorted(records):
    if amp != "none":
        ...
        out_file = args.output_dir + '/reads/' + amp + '.fastq'
...
for amp in sorted(genotype_counter):
    ...
    top_minimap_genotype_per_amp = sorted(minimap_genotype_counter[amp], key = lambda x : minimap_genotype_counter[amp][x], reverse=True)[0]
    print(amp, top_genotype_per_amp, top_minimap_genotype_per_amp)
    fwref = open(args.output_dir+ "/primer-schemes/"+amp+".reference.fasta","w")
    sequence=seq_dict[top_genotype_per_amp]
    fwref.write(">{}\n{}\n".format(top_genotype_per_amp,sequence))
    fwref.close()

    fwbed = open(args.output_dir+"/primer-schemes/"+amp+".scheme.bed","w")
    with open(args.amp_file,"r") as f:
        for l in f:
    ...
    for genotype in sorted(genotype_counter[amp], key = lambda x : genotype_counter[amp][x], reverse=True):
        print("{}\t{}\t{}\t{}\n".format(amp, genotype, genotype_counter[amp][genotype],top_minimap_genotype_per_amp))
        summary_dict[amp].append((genotype,genotype_counter[amp][genotype],top_minimap_genotype_per_amp))

fsum = open(args.summary,"w")
fsum.write("Amplicon,Reference,Count,MinimapHit\n")
